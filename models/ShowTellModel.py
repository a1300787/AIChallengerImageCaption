...
class ShowTellModel(nn.Module):
    ...
    def sample(self, fc_feats, att_feats, opt={}):
        ...
        for t in range(self.seq_length + 2):
            ...
            #print("***********************")
            #print("state:",state)
            output, state = self.core(xt.unsqueeze(0), state)  #  do rnn lstm operation
            #print("##########################################################")
            #print("the oringal of problem:",output)
            #print("XT protental have problem:",xt)
            #print("^^^^^^^^^^^^^^^^^^^^^^")
            #print("stae problem:",state)
            logprobs = F.log_softmax(self.logit(self.dropout(output.squeeze(0))))
            #print("problem is here???:",logprobs)
