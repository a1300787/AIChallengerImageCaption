...
class DataLoader(data.Dataset):
    ...
    def get_vocab(self):
        ...
        return self.ix_to_word  #vocabulary
    ...
    def get_batch(self, split, batch_size=None, seq_per_img=None):
        ...
        fc_batch =    np.ndarray((batch_size * self.seq_per_img, self.fc_feat_size), dtype = 'float32') if self.pre_ft else None
        att_batch =   np.ndarray((batch_size * self.seq_per_img, 14, 14, self.att_feat_size), dtype = 'float32') if self.pre_ft and self.att_im else None
        ...
        for i in range(batch_size):
            ...
            # fetch image
            for fc_num in range(5):
                fc_batch[i*5 + fc_num] = self.h5_fc_file['fc'][ix, :]
                att_batch[i*5 + fc_num] = self.h5_att_file['att'][ix, :, :, :]
            ...
            #label_batch[i * self.seq_per_img: (i + 1) * self.seq_per_img, : self.seq_length ] = seq
...
class DataLoader_pool(data.Dataset):
    ...
    def get_batch_raw(self, split, batch_size=None, seq_per_img=None):
        batch_size = batch_size or self.batch_size
        seq_per_img = seq_per_img or self.seq_per_img
