...
def build_vocab_raw(imgs, params):
...
def encode_captions_raw(imgs, params, wtoi):
    """
...
def encode_captions(imgs, params, wtoi):
    ...
    M = sum(len(img['final_captions'][0]) for img in imgs)  # total number of captions
    ...
    counter = 1
    for i, img in enumerate(imgs):
        n = len(img['final_captions'](0)) #maybe we need know all captions of image
        assert n > 0, 'error: some image has no captions'
...
def main(params):
    ...
    # create the vocab
    vocab = build_vocab_raw(imgs, params)
    ...
    L, label_start_ix, label_end_ix, label_length = encode_captions_raw(imgs, params, wtoi)

    import misc.resnet as resnet
    resnet_type = 'resnet101'
    if resnet_type == 'resnet101':
        resnet = resnet.resnet101()
        resnet.load_state_dict(torch.load('/home/hc/image2txt/chinese_im2text.pytorch/resnet/resnet101.pth'))
    else:
        resnet = resnet.resnet152()
        resnet.load_state_dict(torch.load('/home/hc/image2txt/chinese_im2text.pytorch/resnet/resnet152.pth'))
    ...
    for i, img in enumerate(imgs):
        # load the image
        I = io.imread(os.path.join(params['images_root'], img['filepath'],img['filename']))

        if I.shape[0] > 2048 and I.shape[1]>2048:
...
if __name__ == "__main__":
    ...
    # input json
    parser.add_argument('--input_json', default='/home/hc/image2txt/chinese_im2text.pytorch/data/hc_ai_challenger/coco_ai_challenger.json',
                        help='input json file to process into hdf5')
    parser.add_argument('--num_val', default=30000, type=int,
                        help='number of images to assign to validation data (for CV etc)')
    parser.add_argument('--output_json', default='/home/hc/image_root/ai_challenger/coco_ai_challenger_talk.json', help='output json file')
    parser.add_argument('--output_h5', default='/home/hc/image_root/ai_challenger/coco_ai_challenger_talk', help='output h5 file')

    # options
    parser.add_argument('--max_length', default=36, type=int,
                        help='max length of a caption, in number of words. captions longer than this get clipped.')
    parser.add_argument('--images_root', default='/home/hc/image_root/',
    ...
    parser.add_argument('--model',default='resnet101',help='choose the model type to extract features resnet101/resnet152')
