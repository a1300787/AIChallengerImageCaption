...
def convert2coco_test(img_dir):
    ...
    output_file = os.path.join('/home/hc/image2txt/chinese_im2text.pytorch/data/hc_ai_challenger/', 'ai_challenger_test1.json')
    with open(output_file, 'w') as fid:
        json.dump(coco, fid)
    print('Saved to {}'.format(output_file))
...
def ai_challenger_preprocess():
    ...
    for i, img in enumerate(imgs):
        ...
        #for a in annotsi:
        tmp_sentids = []
        for indic, s in enumerate(annotsi['caption']):
            jimg = {}
            jimg['tokens'] = " ".join(jieba.cut(s)).split(" ")
            jimg['raw'] = s
            jimg['imgid'] = img['id'] + '_' + str(i)
            jimg["sentid"] = indic
            tmp_sentids.append(sentid)
            sentid = sentid + 1
            out_s.append(jimg)
        out_im["imgid"] = img['id']
        out_im['sentences']=out_s
        out_im["sentids"]=tmp_sentids
        out_im['split'] = split
        out.append(out_im)
        #print(out_im)
        #print(len(out))
    ...
    output_file = os.path.join(
        '/home/hc/image2txt/chinese_im2text.pytorch/data/hc_ai_challenger/',
        'coco_ai_challenger.json')
